...
| 单词本 | 从注音原文选词；支持按 JLPT 等级筛选、独立词条详情、收藏、标签和汉字/假名搜索 |
...
- 当前版本：`1.0.8`，最低支持 Android 8.0（API 26）。
- 本地逻辑测试：69 个，覆盖 JLPT 等级筛选、读音候选与选区解析、Ruby 渲染、模型 JSON 校验、句子结构渲染、语境答案归一化和复习调度。
